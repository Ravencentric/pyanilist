...
def test_anilist_get_all_media_docs() -> None:
    ...
    docstring_params = [param.arg_name for param in parse(inspect.getdoc(AniList.get_media_many)).params]  # type: ignore[arg-type]
    async_docstring_params = [param.arg_name for param in parse(inspect.getdoc(AsyncAniList.get_media_many)).params]  # type: ignore[arg-type]
    params = inspect.getargs(AniList.get_media_many.__code__).args[1:]  # exclude the first "self" param
    async_params = inspect.getargs(AsyncAniList.get_media_many.__code__).args[1:]  # exclude the first "self" param
